...
class q_learning_univ(metaclass=StaticMeta): # This algorithm updates element-wise

    @struct.dataclass
    class State:
        q_vals: QType
        gamma: jnp.ndarray
        alpha: jnp.ndarray # Learning rate (scalar)

    def init(mdp: MDP, key: jrd.PRNGKey, gamma: jnp.ndarray, alpha: jnp.ndarray) -> "q_learning_univ.State":
        
        q_vals = jnp.zeros((mdp.state_size, mdp.action_size))
        
        return q_learning_univ.State.replace(q_vals = q_vals)
    
    def compute_delta(state: "q_learning_univ.State", transition: Transition) -> F["A S"]:

        q_vals = state.q_vals
        curr_q = jnp.einsum("as,a,s->", q_vals, transition.reward, transition.state)
        next_q = jnp.max(jnp.einsum("as, s->a", q_vals, transition.next_state))

        td_target = transition.reward + state.gamma*next_q*(1-transition.terminal)
        td_error = td_target - curr_q

        update = jnp.einsum("a,s->as", transition.action, transition.state) # What is this step
        delta = td_error*update

        return delta
    
    def update(state: "q_learning_univ.State", transition: Transition) -> "q_learning_univ.State":

        delta = q_learning_univ.compute_delta(state, transition)
        q_vals = state.q_vals
        q_upd = q_vals + state.alpha*delta

        return state.replace(q_vals = q_upd)