...
class bellman_operator(metaclass=StaticMeta):
    ...
    def v(mdp: MDP, policy: PiType, value: VType, gamma: float) -> VType:
        ...
        target_values = jnp.einsum("axs,x,x->as",
                                   mdp.transition, value, (1 - mdp.terminal)) # (1 - mdp.terminal) considered for making out terminal states as they have zero value
        reward = jnp.einsum("asx,axs->as", mdp.reward, mdp.transition)
...
class bellman_optimality_operator(metaclass=StaticMeta):
    ...
    def v(mdp: MDP, value: VType, gamma: float) -> VType:
        r"""""
        Bellman Optimality Operator for V-Values.

        .. math::
            \mathcal{T}^*(V)(s) = r(s, a) + \gamma \sum_{s'} P(s'|s, a) V(s')
        
            Args:
            mdp (MDP): Markov Decision Process
            value (VType): V Value array
            gamma (float): Discount factor

        Returns:
            VType: Updated V-values after applying Bellman optimality operator

        """

        target_values = jnp.einsum("axs,x->as", mdp.transition, value)
        rewards = jnp.einsum("axs,asx->as", mdp.transition, mdp.reward)
        return jnp.max(rewards + gamma*target_values, axis = 0, keepdims = False)
