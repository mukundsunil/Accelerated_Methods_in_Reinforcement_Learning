...
def plot_comparison(paths, labels, metric='bellman_linf', opt_rho=None):
    ...
    if metric == 'pi_eval_rho':
        plt.ylabel(r'Policy Evaluation $\rho$', fontsize=25)
        ...
        plt.xscale('log')
        plt.ylim(top=1)
        plt.ylabel('Value Norm (Log Scale)', fontsize=25)
        
    elif metric == 'bellman_linf':
        plt.yscale('log')
        plt.xscale('log')
        plt.ylim(bottom=5e-1, top=1)
        plt.ylabel(r'Bellman $L_\infty$ Error (Log Scale)', fontsize=25)

    plt.xlabel('Steps', fontsize=25)
    plt.title(f'Comparison: {metric.replace("_", " ").title()}', fontsize=25)
